...
import { fetchDashboardStats, fetchFingerprintLogs, fetchFaceIdLogs, fetchLessonLogs, fetchClassSchedule, fetchGraduationData, getBaseUrl } from '../services/api';
...
export const DashboardView: React.FC<DashboardViewProps> = ({ user, onLogout }) => {
  ...
  useEffect(() => {
    if (activeTab === 'reports' && user.nis) {
      const getReports = async () => {
        setReportsLoading(true);
        try {
          const [fpData, faceData, lessonData] = await Promise.all([
            fetchFingerprintLogs(user.nis!),
            fetchFaceIdLogs(user.nis!),
            fetchLessonLogs(user.nis!)
          ]);
          setFingerprintLogs(fpData.data || []);
          setFaceIdLogs(faceData.data || []);
          setLessonLogs(lessonData.data || []);
        } catch (err) {
    ...
    if (activeTab === 'academic' && user.idkelas) {
      const getSchedule = async () => {
        setAcademicLoading(true);
        try {
          const data = await fetchClassSchedule(Number(user.idkelas));
          if (data.success) {
    ...
    if (activeTab === 'graduation' && user.nis) {
      const getGraduation = async () => {
        setGraduationLoading(true);
        try {
          const data = await fetchGraduationData(user.nis!);
  ...
  }, [activeTab, user.nis, user.idkelas]);
            ...
            <div className="flex-1">
              ...
              <div className="flex flex-wrap items-center gap-3">
                 ...
                 <div className="bg-white/10 rounded-2xl px-4 py-2 backdrop-blur-md border border-white/10 flex flex-col gap-0.5">
                    <p className="text-[8px] font-black uppercase tracking-widest text-indigo-200">NIS / NISN</p>
                    <p className="text-[10px] font-black uppercase tracking-widest text-indigo-50">
                      {user.nis || user.id} {user.nisn ? `/ ${user.nisn}` : ''}
                    </p>
                 </div>
              </div>
            </div>
            ...
            >
               <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <h3 className="text-sm font-black text-slate-800 uppercase tracking-widest">Jadwal</h3>
                </div>
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center justify-between sticky top-16 bg-slate-50 py-2 z-10 border-b border-slate-100 mb-2">
                      ...
                      <span className="text-[9px] font-bold text-slate-400 bg-slate-100 px-2 py-0.5 rounded-md uppercase">
                        {schedule[selectedScheduleDay]?.length || 0} Sesi
                      </span>
                    </div>
                    <div className="space-y-3 pl-3 border-l-2 border-indigo-100">
                      {schedule[selectedScheduleDay]?.map((session: any, idx: number) => (
                         <motion.div 
                            key={idx}
                         ...
                         >
                            ...
                            <div className="flex items-center gap-1.5">
                               ...
                               <p className="text-[10px] font-medium text-slate-500 italic">{session.nama_guru}</p>
                            </div>
                         </motion.div>
                      ))}
                    </div>
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-40">
        <div className="bg-slate-900/95 backdrop-blur-xl border-t border-white/5 px-2 pt-2 pb-8 flex items-center justify-between">
          ...
          <button 
            ...
            className={`flex flex-col items-center gap-1 p-2 rounded-xl transition-all flex-1 ${activeTab === 'graduation' ? 'bg-indigo-600 text-white' : 'text-slate-400'}`}
          >
